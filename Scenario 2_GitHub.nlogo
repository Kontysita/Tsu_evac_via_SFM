extensions [profiler csv vid ls]
...
to create_adults
  ...
  ask p [ if not any? turtles in-radius 1; patch-ahead 1 ;turtles-on neighbors4 ;turtles-here and patch-ahead 1;;;1 its better than 0.7 as 0.7 produce "oscillations" at the beg of the simulation on vi graph
        [sprout-adults 1
          [ set shape "agent"
        ...
        if (exit-type = 4)[set goal one-of patches with [(pycor > min-pycor + 3 and pycor <= max-pycor - 5 and pxcor = max-pxcor) and pcolor = brown + 2]];3,5
           ...
           ;set vdes (list (vdes_tsu + random-float 0.34) 0)
            ...
            set sigmoid 0.5 + (random-float 0.5)         ; for mixed scenario from intermediate to high
            set susceptibility random 3
         ]
       ]
      ]
end
...
to move
 ...
 [
  ...
  if sigmoid_cao_mod? [calculate-evac_stress]
...
to calculate_crowd_pressure
    set my-neighbors (other turtles) in-radius spread_radius;crowd_press_radius
    set num-neighbors count my-neighbors
    ;show my-neighbors;num-neighbors

    ;;;;to estimate crowd pressure;;;;

  if spread_radius = 1 [set crowd_pressure num-neighbors / 3]

    if spread_radius = 1.1 [set crowd_pressure num-neighbors / 3]

  if spread_radius = 1.5 [set crowd_pressure num-neighbors / 8]

  if spread_radius = 2 [set crowd_pressure num-neighbors / 15]

  if spread_radius = 3 [set crowd_pressure num-neighbors / 35]
end

to stress-spread
  set high_stressed_k (other turtles) in-radius spread_radius with [stress = "high"]; from Dorso, they also tried a radius of 4 and 6. Fig 7
  set n_high_stressed_k count high_stressed_k
  set neigh (other turtles) in-radius spread_radius
  set n count neigh
  ;if n > 0 [set contagion (n_high_stressed_k / n)]
   set contagion ifelse-value (n = 0)
  [0]
  [(n_high_stressed_k / n)]
    ;show [contagion] of turtles
;set contagion 0.3

if susceptibility = 0 [set susceptibility "low"]
          if susceptibility = 1 [set susceptibility "intermediate"]
            if susceptibility = 2 [set susceptibility "high"]

  if susceptibility = "high" and contagion > 0 and stress = "intermediate" [set stress "high"]
   if susceptibility = "intermediate" and contagion > 0.5 and stress = "intermediate" [set stress "high"]
  if susceptibility = "low" and contagion = 1 and stress = "intermediate" [set stress "high"]

  ;if high_susc? [high-susceptibility]
   ;if medium_upper-susc? [medium_upper-susceptibility]
    ;if medium_low-susc? [medium_low-susceptibility]
     ;if low_susc? [low-susceptibility]
end

to stress_percentage
  ;if count turtles = threshold [ask n-of (count turtles * percentage) turtles [set stress "low"]]
  ;if count turtles >= 2 [ask turtles with stress "high" (count turtles * percentage) turtles [set stress "low"]]

  let subset-of-turtles turtles with [ stress = "high"]
if any? subset-of-turtles [
   ; ask max-n-of (count turtles * percentage) subset-of-turtles [vdes] [set stress "low"]
    ;ask max-n-of n subset-of-turtles
  ]
end
...
;  ;clear-patches
;  ask turtles
;    [ paint-agents patches in-radius crowd_press_radius ]
;  display
;end

;to paint-agents [agents]
;  if paint-agents? [ask agents [ if pcolor = black [ set pcolor [color] of myself - 2 ]]];one-of base-colors - 3 ]] ;set pcolor [color] of myself - 1 ]
;end

;to calculate-neighbors
;  let neighborss other turtles in-radius 1
;  ask turtles [count turtles-on neighborss]
;end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;show (list "#: " who " vi: " vi " Fsiw: " Fsiw " Fsiw_obs: " Fsiw_obs " Fsij: " Fsij "Fdes_tsu: " Fdes_tsu);  "Fdes_tsu: " Fdes_tsu)
  ...
  let vix item 0 vi + item 0 Fdes_tsu + item 0 Fsij + item 0 Fsiw + item 0 Fsi_obs + item 0 Fbij + item 0 Fbi_obs + item 0 Fbiw ;+ item 0 Fdes;];[]
  let viy item 1 vi + item 1 Fdes_tsu + item 1 Fsij + item 1 Fsiw + item 1 Fsi_obs + item 1 Fbij + item 1 Fbi_obs + item 1 Fbiw ;+ item 1 Fdes
  ;show list vix viy
 ; print vix
 ; print viy
report list vix viy
...
817
NIL
count turtles with [susceptibility = \"low\"]
17
1
11
...
766
NIL
count turtles with [susceptibility = \"intermediate\"]
17
1
11
...
717
NIL
count turtles with [susceptibility = \"high\"]
17
1
11
...
716
NIL
count turtles with [stress = \"high\"]
17
1
11
...
764
NIL
count turtles with [stress = \"intermediate\"]
17
1
11
...
This model corresponds to Scenario 2 --> Tsunami case considering evacuation stress where agents move at running speeds.
...
To change scenarios, carefully follow the steps in the next section.

## HOW TO USE IT

To test Scenario 2 without stress spread do as follows:

1) Turn on switch sigmoid_cao_mod?

2) Press reset, setup, and go.
...
For the interaction between pedestrians, can run different simulations by varying the FOV_radius and rj_radius. 

For the interaction between pedestrians and obstacles/walls the same can be done by varying the FOV_obs, FOV_walls,angle_walls, and angle_obs.
...
@#$#@#$#@
<experiments>
  <experiment name="testing_R_IV" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    ...
    <metric>[vi] of turtles</metric>
    <metric>[Fsiw] of turtles</metric>
    <metric>[Fsij] of turtles</metric>
    <metric>[Fdes_tsu] of turtles</metric>
    <metric>[sigmoid] of turtles</metric>
    <enumeratedValueSet variable="fps">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="show-FOV?">
      <value value="false"/>
    </enumeratedValueSet>
  ...
  <experiment name="no_tsu_10runs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    ...
    <metric>[xcor] of turtles</metric>
    <metric>[ycor] of turtles</metric>
    <metric>cmax_exit</metric>
    <metric>[vi] of turtles</metric>
    <metric>[Fsij] of turtles</metric>
    <metric>[Fsiw] of turtles</metric>
    <metric>[Fdes] of turtles</metric>
    <metric>[dist_sz] of turtles</metric>
    <metric>[sigmoid] of turtles</metric>
    <metric>[vdes] of turtles</metric>
    <metric>[vdes_notsu] of turtles</metric>
    ...
    <enumeratedValueSet variable="lambda1">
      <value value="0.0017"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="show-FOV?">
      <value value="false"/>
    ...
    <enumeratedValueSet variable="FOV_obj">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold">
      <value value="80"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Fdes_K?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Aiw">
      <value value="2000"/>
    ...
    <enumeratedValueSet variable="lambda0">
      <value value="0.0086"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Ai">
      <value value="2000"/>
  ...
  <experiment name="tsu" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
  ...
  <experiment name="Trying different radius" repetitions="5" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
    <metric>count turtles</metric>
    <metric>count turtles with [pcolor = lime + 1]</metric>
    <metric>[who] of turtles</metric>
    <metric>[xcor] of turtles</metric>
    <metric>[ycor] of turtles</metric>
    <metric>cmax_exit</metric>
  ...
  <experiment name="Different_radius_parameters" repetitions="5" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="2500"/>
  ...
  <experiment name="Interaction peds and walls" repetitions="5" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
    <metric>count turtles</metric>
    <metric>count turtles with [pcolor = lime + 1]</metric>
    <metric>count turtles with [pcolor = cyan + 5]</metric>
    <enumeratedValueSet variable="vdes_notsu">
      <value value="0.66"/>
      <value value="1"/>
    </enumeratedValueSet>
  ...
  <experiment name="Interaction peds_walls_obs and Ai, Bi" repetitions="5" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
    ...
    <enumeratedValueSet variable="angle_walls">
      <value value="200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="angle_obs">
      <value value="200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rj_radius">
      <value value="1.5"/>
    ...
    <enumeratedValueSet variable="FOV_b">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Beta">
      <value value="0.5"/>
  ...
  <experiment name="Checking fd for wall obs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
    ...
    <metric>count turtles with [pcolor = cyan + 5]</metric>
    <metric>[vi] of turtles</metric>
    <metric>[item 0 vi] of turtles</metric>
    <metric>[item 1 vi] of turtles</metric>
    ...
    <enumeratedValueSet variable="fdd">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Fdes_SFM?">
      <value value="true"/>
  ...
  <experiment name="All forces_no_tsu_wall_obs" repetitions="5" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    ...
    <metric>[Fsiw_obs] of turtles</metric>
    <metric>mean [item 0 Fsiw_obs] of turtles</metric>
    <metric>mean [item 1 Fsiw_obs] of turtles</metric>
    <metric>[Fdes] of turtles</metric>
    <metric>mean [item 0 Fdes] of turtles</metric>
    <metric>mean [item 1 Fdes] of turtles</metric>
    <metric>[Fbiw] of turtles</metric>
    <metric>mean [item 0 Fbiw] of turtles</metric>
    <metric>mean [item 1 Fbiw] of turtles</metric>
    <metric>[Fbij] of turtles</metric>
  ...
  <experiment name="Fsij_intimate &lt; 0.46_Johansson" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
  ...
  <experiment name="Fsij_personal [0.46-1.2]_Johansson, A=0.16, B=0.64" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_social [1.2- 2]_Johansson, A=0.16, B=0.64" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_public &gt; 3.7 Johansson both parameters" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_social [1.2- 2]_Johansson, A=0.68, B=2.66" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_personal [0.46-1.2]_Johansson, A=0.68, B=2.66" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_personal [0.46-1.2]_Helbing_2005, A=3, B=0.20" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_social [1.2- 3.7]_Helbing_2005, A=3, B=020" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_personal [0.46-1.2]_Johansson, A=0.68, B=2.66, rj=1" repetitions="30" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_personal+ Fbij [0.46-1.2]_Helbing_2005, A=3, B=0.20" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_social+Fbij [1.2- 3.7]_Helbing_2005, Ai=3, Bi=0.20, Aiw=5, Biw=0.10" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_social+Fbij [1.2- 3.7]_Helbing_2000 m/s2, Ai=3, Bi=0.20, Aiw=27, Biw=0.08" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_social+Fsiw_obs+Fbij_biggerFOV_Helbing_2005, Ai=3, Bi=0.20, Aiw=5, Biw=0.10" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_public+Fsiw_obs+Fbij_biggerFOV_Helbing_2005, Ai=0, Bi=0.20, Aiw=5, Biw=0.10" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij_public+Fsiw_obs+Fbij_Helbing_2005, Ai=0, Bi=0.20, Aiw=5, Biw=0.10" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
  ...
  <experiment name="Fsij+Fsiw_obs final param Aiw=35, Biw=0.13 FOV=1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4500"/>
    <metric>count turtles</metric>
    <metric>count turtles with [pcolor = lime + 1]</metric>
    <metric>[who] of turtles</metric>
    <metric>mean [item 0 Fsij] of turtles</metric>
  ...
  <experiment name="Fsij+Fsiw_obs final param Aiw=35, Biw=0.13 FOV=2" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4500"/>
  ...
  <experiment name="Fsij+Fsiw_obs final param Aiw=35, Biw=0.13 FOV=3" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4500"/>
  ...
  <experiment name="Fsij+Fsiw_obs final param Aiw=35, Biw=0.13 FOV=4" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4500"/>
  ...
  <experiment name="Proxemics 50 agents env0, env1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="3000"/>
  ...
  <experiment name="Proxemics 100 agents env0, env1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
  ...
  <experiment name="Proxemics 200 agents env0, env1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="5000"/>
...
setup</setup>
    ...
    <metric>count turtles</metric>
    <metric>exit_density</metric>
    <metric>[n] of turtle 1</metric>
    <metric>mean [n] of turtle 1</metric>
    <metric>[n_high_stressed_k] of turtle 1</metric>
    <metric>mean [n_high_stressed_k] of turtle 1</metric>
    <metric>[contagion] of turtle 1</metric>
    <metric>mean [contagion] of turtle 1</metric>
    <enumeratedValueSet variable="vdes_notsu">
      <value value="0.66"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="tickss">
  ...
  <experiment name="50_agents_env1_env0_tsu_rj=1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
  ...
  <experiment name="100_agents_env1_env0_tsu_rj=3" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="200_agents_env1_env0_tsu_rj=3" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="50_agents_env1_env0_tsu_rj=3" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="200_agents_env1_env0_tsu_rj=1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="100_agents_env1_env0_tsu_rj=1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="100_agents_env0_tsu_rj=1Beta testing" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="100_agents_env0_tsu_rj=1Beta testingII" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="200,100 and 50 agents_random_sigmoid_3env_tsu" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1500"/>
    ...
    <metric>density_obs14</metric>
    <metric>density_obs15</metric>
    <metric>mean [num-neighbors] of turtles</metric>
    <metric>[stress] of turtles</metric>
    <metric>mean [diwall] of turtles</metric>
    <enumeratedValueSet variable="tickss">
      <value value="400"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rj_radius">
  ...
  <experiment name="200 agents_env0_stress_spread_high_susceptibility" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
    ...
    <metric>density_obs14</metric>
    <metric>density_obs15</metric>
    <metric>mean [n] of turtles</metric>
    <metric>mean [n_high_stressed_k] of turtles</metric>
    <metric>mean [contagion] of turtles</metric>
    <enumeratedValueSet variable="vdes_notsu">
      <value value="0.66"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="tickss">
    ...
    <enumeratedValueSet variable="spread_radius">
      <value value="1"/>
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rj_radius">
  ...
  <experiment name="200 agents_2env_stress_spread_medium_upper_susceptibility" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="200 agents_2env_no_stress_spread" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="200 agents_2env_stress_spread_low_susceptibility" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="200 agents_2env_no_stress_spread_real_sigmoid" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
    ...
    <metric>mean [n_high_stressed_k] of turtles</metric>
    <metric>mean [contagion] of turtles</metric>
    <metric>mean [sigmoid] of turtles</metric>
    <enumeratedValueSet variable="tickss">
      <value value="400"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="spread_radius">
  ...
  <experiment name="Checking Fbij on-off" repetitions="5" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1500"/>
    ...
    <metric>mean [sigmoid] of turtles</metric>
    <metric>count turtles with [pcolor = lime  + 1]</metric>
    <metric>[who] of turtles with [pcolor = lime  + 1]</metric>
    <metric>[item 0 Fbij] of turtles with [pcolor = lime  + 1]</metric>
    <metric>[item 1 Fbij] of turtles with [pcolor = lime  + 1]</metric>
    ...
    <enumeratedValueSet variable="high_susc?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="spread_radius">
      <value value="1.1"/>
    ...
    <enumeratedValueSet variable="medium_upper-susc?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Beta">
      <value value="0.5"/>
    ...
    <enumeratedValueSet variable="medium_low-susc?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="low_susc?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="env-type">
      <value value="1"/>
  ...
  <experiment name="Sensitivy_analysis_tsu_env3_exit0_no_obs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
  ...
  <experiment name="Sensitivy_analysis_tsu_env1_exit0_obs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="Sensitivy_analysis_tsu_env1_exit0_obs. Fsij Helbing 2005. Fsi_obs Helbing 2000mod" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="Sigmoid_no_obs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="Sigmoid_obs_env1" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
...
setup</setup>
  ...
  <experiment name="50 agents_tsu_env0_exit0" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="50 agents_tsu_env1_exit0_obs" repetitions="30" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="100 agents_tsu_env0_exit0" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="100 agents_tsu_env1_exit0_obs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="200 agents_tsu_env0_exit0" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="200 agents_tsu_env1_exit0_obs" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="Fbij_valores_per_agent" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count turtles</metric>
    ...
    <metric>fbijyy</metric>
    <metric>id2</metric>
    <metric>fbijxx2</metric>
    <metric>fbijyy2</metric>
    <enumeratedValueSet variable="vdes_notsu">
      <value value="0.66"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="tickss">
    ...
    <enumeratedValueSet variable="paint-agents?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="spread_radius">
      <value value="1.1"/>
    ...
    <enumeratedValueSet variable="percentage">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pen-down?">
      <value value="false"/>
    ...
    <enumeratedValueSet variable="ticks-for-threshold?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="lambda0">
      <value value="0.0055"/>
  ...
  <experiment name="450 agents tsu" repetitions="1" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="1200"/>
  ...
  <experiment name="Stress_spread_env0_exit0_rad=0,1,2,3" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    ...
    <metric>mean [contagion] of turtles</metric>
    <metric>mean [num-neighbors] of turtles</metric>
    <metric>mean [crowd_pressure] of turtles</metric>
    <metric>count turtles with [stress = "high"]</metric>
    <metric>count turtles with [stress = "intermediate"]</metric>
    <metric>count turtles with [susceptibility = "low"]</metric>
    <metric>count turtles with [susceptibility = "intermediate"]</metric>
    <metric>count turtles with [susceptibility = "high"]</metric>
    <enumeratedValueSet variable="tickss">
      <value value="500"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="spread_radius">
  ...
  <experiment name="Stress_spread_env1_exit0_rad=0,1,2,3" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
  ...
  <experiment name="Stress_spread_env0_exit0_rad=0,1,2,3 50 agents" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
  ...
  <experiment name="Stress_spread_env1_exit0_rad=0,1,2,3 50 agents" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
  ...
  <experiment name="Stress_spread_env0_exit0_rad=0,1,2,3 100 agents" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
  ...
  <experiment name="Stress_spread_env1_exit0_rad=0,1,2,3 100 agents" repetitions="10" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="4000"/>
  ...
  <experiment name="Stress_spread_200_agents_env0_exit0_rad=2" repetitions="100" runMetricsEveryStep="true">
    <setup>reset
setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
